 ...
 *
 * The layout is the deterministic radial arrangement validated offline: a hub
 * in the middle, one radius band per BFS layer, angular sectors per
 * relationship role, ring-capacity spacing and a minimum spoke angle.
 *
 * This file is intentionally plain (no JSX, no bundler): the Loader contract
 * only needs a single script calling `window.__ModuleLoader__.load`.
 */
    ...
    function ResearchCatPanel() {
      ...
      React.useEffect(function () {
        ...
        post('state', {}).then(function (res) {
          if (alive && res && res.graph) setGraph(res.graph)
        }, function () {})
      ...
      function runSearch(term) {
        ...
        setError('')
        post('search', { query: asked }).then(function (res) {
          setResults(res.results || [])
          setResultsFor(res.term || asked)
      ...
      function expand(id, kind) {
        ...
        setError('')
        post('expand', { id: id, kind: kind }).then(function (res) {
          setGraph(res.graph)
      ...
      function expandAll() {
        ...
        setError('')
        post('expandAll', {}).then(function (res) {
          setGraph(res.graph)
